...
Element | Meaning
:--------: | :--------:
`mark({mark,loc||count})` | Marker element placing marker symbol `mrk` at locations `loc`. If `count` is given instead of `loc`, the marks are distributed evenly on the respective element.
